 ...
 * CHANGELOG.md:
 *
 *   - Use non-qualified names in the object reference when the extension is
 *     not bound to a specific `schema` via its `.control` file.
 */
...
begin
    ...
    if collection_type$ = 'extension' then
        ...
        if _extension_schema is not null then
            ...
            perform set_config(
                'search_path',
                concat_ws(',', _extension_schema, nullif(current_setting('search_path', false), '')),
                true
            );
        end if;
    end if;
    ...
    if array_length((objects$).procedure_objects, 1) > 0 then
        ...
        loop
            ...
            then
                _text := _text || E'```sql\n'
                    || case
                            when _extension_schema is not null then
...
```sql
CREATE OR REPLACE PROCEDURE test__my_view()
 LANGUAGE plpgsql
 SET search_path TO 'readme', 'pg_temp'
AS $procedure$
declare
